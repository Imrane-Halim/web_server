...
class CGIHandler : public EventHandler
{
    private:
        ...
        void initArgv();
    public:
        CGIHandler(const std::string &scriptPath, HTTPParser &parser, FdManager &fdm);
        ~CGIHandler() {}
        int get_fd();
        void start();
        void onReadable();
...
CGIHandler::CGIHandler(const std::string &scriptPath, HTTPParser &parser, FdManager &fdm)
    : _scriptPath(scriptPath), _pid(-1), _isRunning(false) , _parser(parser), EventHandler(fdm)
{
